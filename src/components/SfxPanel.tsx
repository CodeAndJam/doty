...
export default function SfxPanel({ sfxRecommendations }: Props) {
  ...
  // Persist pins
  useEffect(() => { saveSfxPins(pinned) }, [pinned])
  ...
  return (
    <div className="h-full flex flex-col relative">
        ...
        <div className="mb-2 shrink-0">
          <div className="flex items-center gap-1 overflow-x-auto pb-1">
            {sfxPlayer.channels.map(ch => (
              ...
              >
                {ch.looping && (
                  <span style={{ fontSize: '8px', color: '#4a8a6a' }}>&#x21BB;</span>
                )}
                <span className="truncate" style={{ maxWidth: '80px' }}>{ch.label}</span>
                <button
      ...
      <div className="flex items-center gap-2 mb-2 shrink-0">
        <span style={{ fontSize: '10px', fontFamily: 'monospace', color: '#3a2e1a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          SFX Vol
        </span>
        <input
          type="range"
        ...
        />
        <span style={{ fontSize: '10px', fontFamily: 'monospace', color: '#3a2e1a', minWidth: '28px', textAlign: 'right' }}>
      ...
      <div className="flex items-center gap-2 mb-2 shrink-0 overflow-x-auto">
        ...
        {/* Search */}
        <div className="flex items-center gap-1 ml-auto shrink-0" style={{
      ...
      {allSfx.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center relative" style={{
          ...
          <div className="absolute top-0 left-0 w-3 h-3" style={{ borderTop: '1px solid rgba(74,138,106,0.3)', borderLeft: '1px solid rgba(74,138,106,0.3)' }} />
          <div className="absolute bottom-0 right-0 w-3 h-3" style={{ borderBottom: '1px solid rgba(74,138,106,0.3)', borderRight: '1px solid rgba(74,138,106,0.3)' }} />
          ...
          <p style={{ fontFamily: "'Cinzel', serif", fontSize: '14px', color: '#4a8a6a', letterSpacing: '0.15em', textAlign: 'center', opacity: 0.6 }}>
            No sound effects available
          </p>
          <p style={{ marginTop: '8px', fontSize: '13px', color: '#3a2e1a', fontFamily: "'Crimson Text', serif", textAlign: 'center' }}>
          ...
          {pinnedSfx.length > 0 && (
            <>
              <div className="flex items-center gap-2 shrink-0 mb-1" style={{ height: '20px' }}>
                <PinIcon filled />
                <span style={{ fontFamily: "'Cinzel', serif", fontSize: '11px', letterSpacing: '0.2em', color: '#4a8a6a', textTransform: 'uppercase' }}>
                  Pinned
                </span>
                <div className="flex-1 h-px" style={{ background: 'rgba(74,138,106,0.15)' }} />
              </div>
              <div className="grid grid-cols-2 gap-1 mb-2">
                {pinnedSfx.map(renderSfxCard)}
              </div>
            </>
          )}
              ...
              {pinnedSfx.length > 0 && (
                <div className="flex items-center gap-2 shrink-0 mb-1" style={{ height: '20px' }}>
                  ...
                  <span style={{ fontFamily: "'Cinzel', serif", fontSize: '11px', letterSpacing: '0.2em', color: '#3a2e1a', textTransform: 'uppercase' }}>
                    {selectedCategory === 'recommended' ? 'Suggested' : selectedCategory === 'all' ? 'All Effects' : SFX_CATEGORY_LABELS[selectedCategory as SfxCategory]?.split(' ')[0] ?? 'Effects'}
                  </span>
                  <div className="flex-1 h-px" style={{ background: 'rgba(46,36,22,0.5)' }} />
                </div>
              )}
              <div className="grid grid-cols-2 gap-1">
                {filteredSfx.map(renderSfxCard)}
              </div>
            </>
          ) : pinnedSfx.length === 0 ? (
